...
This script trains an LSTM model to predict the next timestep's latent vector
...
from src.time_prediction.LSTM_model import build_lstm_model, get_device
from src.time_prediction.LSTM_data_prep import prepare_lstm_data
...
def main():
    ...
    print(f"Time window (time_sol): {time_sol}")
    print(f"Validation split: {val_split}")
    print("=" * 60)
    
    # Load and prepare data
    print("\nLoading and preparing data...")
    train_input, train_output, val_input, val_output, norm_params = prepare_lstm_data(
        val_split=val_split,
        random_seed=42
    )
    ...
    # Build model
    print("\nBuilding LSTM model...")
    model, device = build_lstm_model(latent_dim=latent_dim, device=device)
    ...
    output_dir = time_prediction_dir / 'output'
    ...
    np.save(scaling_dir / 'param_norm_LSTM.npy', norm_params)
    print(f"Saved parameter normalization to {scaling_dir / 'param_norm_LSTM.npy'}")
    ...
    for epoch in range(epochs):
        ...
        if val_loader is not None and val_loss < best_val_loss:
            ...
            # Save model weights
            torch.save(model.state_dict(), weights_dir / 'weights_LSTM.pth')
        else:
            patience_counter += 1
    ...
    if val_loader is not None:
        model.load_state_dict(torch.load(weights_dir / 'weights_LSTM.pth', weights_only=True))
    
    # Save final model
    torch.save(model.state_dict(), weights_dir / 'weights_LSTM_final.pth')
    print(f"Saved model weights to {weights_dir / 'weights_LSTM.pth'}")
    ...
    history_file = results_dir / 'LSTM.json'
